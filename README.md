...
- **局域网互联** — 设置页连接同一局域网内的电脑端 OhMyMeme（UDP 发现 + AES-GCM 加密会话）：扫描发现电脑、输入密钥配对、从电脑拉取表情、把手机表情上传到电脑、双向同步配置（默认剔除密钥字段），对齐桌面端 `lan.py` 协议
...
- [x] 局域网互联：设置页连接电脑端（UDP 发现 / 密钥配对 / 拉取 / 上传 / 配置同步）
